...
describe('SiteFooter', () => {
  ...
  it('copies the public email, announces success, and resets the control', async () => {
    ...
    const actions = container.querySelector('[data-footer-email-actions]');
    if (!actions) throw new Error('Missing legacy email actions');
    const legacy = within(actions as HTMLElement);
    fireEvent.click(legacy.getByRole('button', { name: 'Copy email address' }));

    await act(async () => {
      await Promise.resolve();
    });
    expect(writeText).toHaveBeenCalledWith('amanda.yangj@gmail.com');
    expect(legacy.getByRole('button', { name: 'Email copied' })).toHaveAttribute(
      'data-copy-state',
      'copied',
    );
    expect(actions.querySelector('[data-footer-email-icon="check"]')).toHaveAttribute(
      'width',
      '16',
    );
    expect(legacy.getByRole('status')).toHaveTextContent('Email copied');

    act(() => vi.advanceTimersByTime(1800));
    expect(legacy.getByRole('button', { name: 'Copy email address' })).toHaveAttribute(
      'data-copy-state',
      'idle',
    );
  });
  ...
  it('keeps the email usable and announces a localized copy failure', async () => {
    ...
    if (!actions) throw new Error('Missing legacy email actions');
    const legacy = within(actions as HTMLElement);
    fireEvent.click(legacy.getByRole('button', { name: '复制邮箱' }));

    expect(
      await legacy.findByRole('button', { name: '复制失败，请手动复制' }),
    ).toHaveAttribute('data-copy-state', 'failed');
    expect(legacy.getByRole('status')).toHaveTextContent('复制失败，请手动复制');
    expect(legacy.getByRole('link', { name: 'amanda.yangj@gmail.com' })).toHaveAttribute(
      'href',
