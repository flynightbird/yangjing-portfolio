...
describe('ConvoAI publication contract', () => {
  ...
  it('publishes the approved Chinese decision-led narrative without retired claims', () => {
    ...
    expect(source).toContain('<ConvoAiContextOverview locale="zh"');
    expect(source).not.toContain('<strong>Recovery</strong>');
    const stateFlow = source.split('\n').find((line) => line.includes('className="state-flow"')) ?? '';
    expect(stateFlow.match(/<strong>/g)).toHaveLength(6);
    expect(source).not.toContain("ids={['app-conversation-start', 'web-login', 'web-preflight', 'web-preflight-layout', 'web-join-exit']}");
    expect(source).toContain('没有输入框，如何让用户掌握话轮？');
    expect(source).toContain('把后台链路翻译成可读的现场');
    expect(source).toContain('同步表达当前话轮');
    expect(source).toContain('找到 Agent 并开始对话”为主线');
    expect(source.match(/声纹解决的不是识别，而是“该听谁”/g)).toHaveLength(1);
